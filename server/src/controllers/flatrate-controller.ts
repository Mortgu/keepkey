...
export const getFlatRate = async (request: Request, response: Response) => {
    ...
    if (!flatrate) {
        return response.status(404).json({ success: false, message: 'Flat rate not found.' });
    }

    return response.status(200).json(flatrate);
};

export const createFlatRate = async (request: Request, response: Response, next: NextFunction) => {
    try {
        const { body } = request;
...
export const updateFlatRate = async (request: Request, response: Response, next: NextFunction) => {
    try {
        const id = request.params.id as string;
        const { body } = request;
...
export const deleteFlatRate = async (request: Request, response: Response, next: NextFunction) => {
    try {
        const id = request.params.id as string;
