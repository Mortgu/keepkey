...
export async function offerReservationJob(task: Task, chainGenerationOnSuccess?: boolean) {
    ...
    await repository.createReservation(env.NEXTCLOUD_OFFER_ORIGINAL_PATH, offer.quoteId);
    await repository.createReservation(env.NEXTCLOUD_OFFER_PDF_PATH, offer.quoteId);
