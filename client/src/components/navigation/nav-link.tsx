...
function NavItemContent({icon, label, badge, trailing, isActive}: NavItemContentProps) {
    return (
        <>
...
export const NavLink: LinkComponent<typeof BasicLinkComponent> = (props) => {
    return (
        <CreatedLinkComponent activeOptions={{exact: true}} {...props} activeProps={{
...
export const NavButton = React.forwardRef<HTMLButtonElement, NavButtonProps>(
    ({icon, label, badge, trailing, isActive, indent, className, ...props}, ref) => (
        <button type="button" ref={ref} {...props} className={itemStyles({indent, isActive, className})}>
            <NavItemContent label={label} icon={icon} badge={badge}
                            trailing={trailing} isActive={isActive}/>
        </button>
    ),
);
...
export function NavGroup({icon, label, badge, defaultOpen = false, isActive, children}: NavGroupProps) {
    const [open, setOpen] = React.useState(defaultOpen);
    return (
        <>
            <NavButton label={label} icon={icon} badge={badge} isActive={isActive}
                       onClick={() => setOpen((v) => !v)}
                       trailing={
