import {Delete, Loader, Plus, Trash} from "lucide-react";
        ...
        <div>
            ...
            <div className='grid gap-2'>
                {data.map((contract: Contract) => (
                    <div key={contract.id} className="rounded-md flex items-center justify-between gap-4 p-2 border border-gray-300 cursor-pointer">
                        {contract.name}
                ...
                {isOpen && (
                    <form onSubmit={(e) => {
                        ...
                        <Button onClick={() => setOpen(false)} type='button' size='sm' variant='secondary'>Cancel</Button>
                    </form>
                )}
            </div>
        </div>
