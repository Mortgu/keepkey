...
export default function TariffComponent(props: Props) {
    ...
    const addBand = () => {
        ...
        onAddBand({tariffId: tariff.id, min_quantity: minQty, max_quantity: maxQty, prices: tariff.terms.map(() => 0)});
    };
    ...
    return (
        <div className="grid bg-white border border-(--border) rounded-md shadow-xs">
            <div className="flex items-center justify-between border-b border-(--border)">
                <div
                    className="w-full flex items-center gap-2 py-4 px-5 hover:bg-(--page-bg) hover:cursor-pointer border-r border-(--border)">
                ...
                <div className="flex items-center gap-2 px-2">
                    ...
                    <Button variant="secondary" size="sm">Neu</Button>
                </div>
            </div>
            ...
            <div className="w-full grid gap-2 p-4">
                <div className="w-full flex gap-2 overflow-x-scroll">
                    <div className="grid gap-2">
                        ...
                        {tariff.rows.map((row) => (
                            <TariffRowComponent key={row.id} row={row} cells={row.cells}
                                                onRemove={() => onRemoveBand(row.id)}
                                                onUpdateCell={(cellId, price) => onUpdateCell({cellId, price})}/>
                        ))}
                    </div>

                    <Button className="h-full border-dashed" icon={<Plus className="size-4"/>} size="sm"
                            variant="secondary" onClick={() => addTerm(12)}/>
                </div>

                <Button variant="secondary" className="w-full border-dashed" onClick={addBand}>
                    Zeile hinzufügen
                </Button>
            </div>
        </div>
