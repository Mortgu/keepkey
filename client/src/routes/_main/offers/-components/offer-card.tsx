...
import {AlertTriangle, Check, Loader, Pen, RotateCcw, Trash} from "lucide-react";
...
export default function OfferCard({offer, onEdit}: OfferListItemProps) {
    const {customerContactPerson: ccp, offerPositions, offerFlatRates} = offer;
    ...
    const {data: taskData,} = useQuery({
        queryKey: ['reservation', reservationTask?.id],
        queryFn: async () => api<Task>(`/api/tasks/${reservationTask?.id}`, {method: 'GET'}),
        ...
        enabled: reservationTask !== undefined && reservationTask?.status !== "COMPLETED",
    });
            ...
            <div className="border border-(--border) rounded-md">
                <div className="flex items-center justify-between px-4 py-3 border-b border-(--border) relative">
                    <div className="grid gap-1">
                        <div className="flex items-center gap-2">
                            ...
                            {reservationTask && (reservationTask.status === "COMPLETED") && (
                                <Badge variant="generated" className='flex items-center gap-1'>
                ...
                <Collapsable label="Produkte" className="w-full bg-(--subtle-50) justify-between rounded-none">
                    <div className="grid gap-2 px-4 py-3">
                        ...
                        {/* Total price */}
                        <div
                            className="flex items-center gap-2 border border-(--border) py-2 px-3 justify-end rounded-md">
                ...
                <Collapsable label="Dokumente" className="w-full bg-(--subtle-50) justify-between rounded-none">
                    ...
                    {offer.documents.length <= 0 && (
                        <div className='grid gap-2 px-4 py-3'>
                            <Button variant="primary" size="sm">dawdaw</Button>
                        </div>
                    )}
                </Collapsable>
